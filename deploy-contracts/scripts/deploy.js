...
async function main() {
  ...
  // Get the DexaCreator address as payable
  const dexaCreatorAddress = await dexaCreator.getAddress();

  // Deploy DexaFeeds
  const DexaFeeds = await hre.ethers.getContractFactory("DexaFeeds");
  const dexaFeeds = await DexaFeeds.deploy(dexaCreatorAddress, await feedsToken.getAddress());
  ...
  const dexaMessenger = await DexaMessenger.deploy(dexaCreatorAddress);
